...
def verify_eddsa_token(token: str, jwk: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        logger.info("🔐 Starting EdDSA token verification...")
        ...
        logger.info(f"Public key (x): {x_b64}")

        # Add proper padding for base64 URL decoding
        x_b64_padded = x_b64 + '=' * (4 - len(x_b64) % 4)
        x_bytes = base64.urlsafe_b64decode(x_b64_padded)

        logger.info(f"Decoded public key length: {len(x_bytes)} bytes")

        # Create the Ed25519 public key
        public_key = ed25519.Ed25519PublicKey.from_public_bytes(x_bytes)
        logger.info("✓ Ed25519 public key created")
        ...
        logger.info(f"Token parts: header={parts[0][:20]}..., payload={parts[1][:20]}..., signature={parts[2][:20]}...")
        ...
        logger.info(f"✓ Successfully verified EdDSA token for user: {payload.get('sub', 'unknown')}, email: {payload.get('email', 'unknown')}")
        return payload
    except Exception as e:
        logger.error(f"❌ EdDSA token verification failed: {e}")
        import traceback
        logger.error(f"EdDSA verification traceback: {traceback.format_exc()}")
        return None