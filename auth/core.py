...
def decode_jwt_token(token: str, secret: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if alg == "EdDSA":
            # EdDSA algorithm - use JWKS to get public key
            logger.info(f"Attempting to verify EdDSA token with JWKS")
            jwk = get_jwk_for_token(token)
            if jwk:
                payload = verify_eddsa_token(token, jwk)
                if payload:
                    logger.info(f"EdDSA token verified successfully - sub: {payload.get('sub')}, email: {payload.get('email')}")
                    return payload
                else:
            ...
            # HMAC algorithms - use the shared secret (fallback)
            payload = jwt.decode(
                token,
                secret,
                algorithms=[alg]
...
async def get_current_user(
    ...
    logger.info(f"🎫 Full token length: {len(token)} chars")

    # Get the secret from request state or environment
    ...
    logger.info("🔍 Starting token decode and verification...")
